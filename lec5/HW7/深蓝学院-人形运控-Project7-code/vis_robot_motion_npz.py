...
def load_any_motion(file_path):
    if file_path.endswith(".npz"):
        data = np.load(file_path, allow_pickle=True)
        motion_fps = data["fps"]
        motion_root_pos = data["root_pos"]
        motion_root_rot = data["root_rot"]
        motion_root_rot = motion_root_rot[:, [3, 0, 1, 2]]
        motion_dof_pos = data["dof_pos"]
        motion_local_body_pos = data["local_body_pos"]
        motion_link_body_list = data["link_body_list"]
        motion_data = data
        ...
        motion_local_body_pos = motion_data["motion_local_body_pos"]
        motion_link_body_list = motion_data["motion_link_body_list"]
    else:
        raise ValueError("仅支持 .npz / .pkl 文件")
...
if __name__ == "__main__":
    ...
    motion_data, motion_fps, motion_root_pos, motion_root_rot, motion_dof_pos, motion_local_body_pos, motion_link_body_list = load_any_motion(robot_motion_path)
