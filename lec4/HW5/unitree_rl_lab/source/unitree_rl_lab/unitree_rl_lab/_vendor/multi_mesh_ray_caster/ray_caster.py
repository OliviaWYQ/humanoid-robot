...
from .warp import convert_to_warp_mesh, raycast_mesh
...
class RayCaster(SensorBase):
    ...
    def _initialize_impl(self):
        ...
        if prim is None:
            available_prims = ",".join([str(p.GetPath()) for p in sim_compat.get_current_stage().Traverse()])
            raise RuntimeError(
    ...
    def _obtain_trackable_prim_view(
        ...
        for mesh_prim, view_prim in zip(mesh_prims, view_prims):
            pos, orientation = sim_compat.resolve_prim_pose(mesh_prim, view_prim)
