...
from . import sim_compat
from .mesh import PRIMITIVE_MESH_TYPES, create_trimesh_from_geom_mesh, create_trimesh_from_geom_shape
...
from .warp import convert_to_warp_mesh, raycast_dynamic_meshes
...
class MultiMeshRayCaster(RayCaster):
    ...
    def __init__(self, cfg: MultiMeshRayCasterCfg):
        ...
        # Diagnostic switch: MMC_DISABLE_TRACKED_VIEWS=1 forces all targets to be treated as
        # static (poses resolved once at init from USD), skipping creation of the regex physics
        # tensor views (RigidBodyView/ArticulationView over ~165k obstacle bodies).
        if os.environ.get("MMC_DISABLE_TRACKED_VIEWS", "0") == "1":
            logger.warning("MMC_DISABLE_TRACKED_VIEWS=1: tracked physics views disabled (diagnostic mode).")
            for target_cfg in self._raycast_targets_cfg:
                target_cfg.track_mesh_transforms = False
    ...
    def _initialize_warp_meshes(self):
        ...
        for target_cfg in self._raycast_targets_cfg:
            ...
            for target_prim in target_prims:
                ...
                for mesh_prim in mesh_prims:
                    ...
                    scale = sim_compat.resolve_prim_scale(mesh_prim)
                    mesh.apply_scale(scale)

                    relative_pos, relative_quat = sim_compat.resolve_prim_pose(mesh_prim, target_prim)
        ...
        for target_cfg in self._raycast_targets_cfg:
            ...
            for prim in sim_utils.find_matching_prims(target_cfg.prim_expr):
                translation, quat = sim_compat.resolve_prim_pose(prim)
