...
class MotionCommand(CommandTerm):
    ...
    def _adaptive_sampling(self, env_ids: torch.Tensor):
        ### TODO 1: Implement adaptive motion sampling / 实现自适应动作采样
        # 1. Read failed environments / 读取失败环境:
        #    Use `self._env.termination_manager.terminated[env_ids]` to get a bool mask.
        #    `env_ids` is the list/tensor of environments that are being resampled.
        #    使用 `self._env.termination_manager.terminated[env_ids]` 得到布尔失败掩码。
        #    `env_ids` 表示本次需要重新采样动作起始帧的环境编号。
        #
        # 2. Map time steps to bins / 将当前动作帧映射到bin:
        #    Formula hint:
        #    `(self.time_steps * self.bin_count) // max(self.motion.time_step_total, 1)`.
        #    Clamp the result to `[0, self.bin_count - 1]`.
        #    公式提示同上，最后用 `torch.clamp` 限制到合法bin范围。
        #
        # 3. Count failed bins / 统计失败bin:
        #    Select failed bins by `current_bin_index[env_ids][episode_failed]`.
        #    Write `torch.bincount(..., minlength=self.bin_count)` into
        #    `self._current_bin_failed[:]`.
        #    只统计本次失败环境所在的bin，并写回 `_current_bin_failed`。
        #
        # 4. Build base probabilities / 构造基础采样概率:
        #    Start from `self.bin_failed_count`, then add
        #    `self.cfg.adaptive_uniform_ratio / float(self.bin_count)`.
        #    这样即使某个bin历史失败数为0，也仍有被采样的概率。
        #
        # 5. Smooth probabilities / 平滑采样概率:
        #    Reshape probabilities to `(1, 1, self.bin_count)`, pad the right side by
        #    `self.cfg.adaptive_kernel_size - 1`, then apply
        #    `torch.nn.functional.conv1d(..., self.kernel.view(1, 1, -1))`.
        #    这一步对应参考实现里的非因果扩散核，让失败帧附近也更容易被采到。
        #
        # 6. Normalize probabilities / 归一化概率:
        #    Divide by `sampling_probabilities.sum()` so the final probabilities sum to 1.
        #
        # 7. Sample bins and convert to time steps / 采样bin并转换回动作帧:
        #    Use `torch.multinomial(sampling_probabilities, len(env_ids), replacement=True)`.
        #    Add a random offset from `sample_uniform(0.0, 1.0, (len(env_ids),), device=self.device)`.
        #    Convert the sampled bin to `[0, self.motion.time_step_total - 1]`,
        #    cast to `.long()`, and assign to `self.time_steps[env_ids]`.
        #
        # 8. Update sampling metrics / 更新采样指标:
        #    Compute normalized entropy, top1 probability, and top1 bin.
        #    Write them into `self.metrics["sampling_entropy"]`,
        #    `self.metrics["sampling_top1_prob"]`, and `self.metrics["sampling_top1_bin"]`.
        raise NotImplementedError("TODO 1: implement _adaptive_sampling")
    ...
    def _update_command(self):
        ### TODO 2: Implement command update / 实现命令逐步更新
        # 1. Advance motion time / 推进动作时间:
        #    Increase every environment time step with `self.time_steps += 1`.
        #
        # 2. Resample finished motions / 重采样已经播放结束的环境:
        #    Find environments where `self.time_steps >= self.motion.time_step_total`.
        #    If any exist, call `self._resample_command(env_ids)`.
        #    注意：`_resample_command` 内部会保持 `sampling_mode` 的分支行为。
        #
        # 3. Repeat anchor tensors / 扩展anchor张量:
        #    Repeat `self.anchor_pos_w`, `self.anchor_quat_w`,
        #    `self.robot_anchor_pos_w`, and `self.robot_anchor_quat_w` from
        #    `(num_envs, 3 or 4)` to `(num_envs, len(self.cfg.body_names), 3 or 4)`.
        #    后续每个body都要用同一个anchor对齐，所以要扩展到body维度。
        #
        ...
        #    This keeps x/y following the robot while preserving reference motion height.
        #    x/y跟随当前机器人位置，z使用参考动作高度。
        #
        # 5. Compute yaw-only rotation alignment / 计算只保留yaw的旋转对齐:
        #    Use `quat_mul(robot_anchor_quat_w_repeat, quat_inv(anchor_quat_w_repeat))`
        #    to get the rotation difference, then wrap it with `yaw_quat(...)`.
        #    只保留heading/yaw，避免roll/pitch误差把参考动作整体拧歪。
        #
        ...
        #    Only when `self.cfg.sampling_mode == "adaptive"`, update
        #    `self.bin_failed_count` using EMA:
        #    `adaptive_alpha * self._current_bin_failed + (1 - adaptive_alpha) * self.bin_failed_count`.
        #    Then call `self._current_bin_failed.zero_()`.
        raise NotImplementedError("TODO 2: implement _update_command")
